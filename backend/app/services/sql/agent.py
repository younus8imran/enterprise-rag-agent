...
class SQLAgent:
    ...
    def _clean_sql(self, raw: str) -> str:
        """Extract only the SELECT SQL statement, stripping LLM explanation text."""
        # Remove markdown code fences
        cleaned = raw.strip().strip("```sql").strip("```").strip()
        # Find first SELECT, keep through last semicolon or end
        ...
        for line in lines:
            ...
            if not started:
                if stripped.upper().startswith("SELECT") or stripped.upper().startswith("WITH"):
                    ...
                    # Skip explanation lines before SQL
                    continue
            else:
                if stripped:
                    sql_lines.append(stripped)
                # Stop at first complete statement if explanation follows
        # Rebuild: keep only the SQL statement part (before any obvious explanation markers)
        result = " ".join(sql_lines)
        # Trim at common explanation markers (e.g., "Note:", "Note that", "This query")
        for marker in ["Note:", "Note that", "This query", "The query", "Corrected SQL", "Based on"]:
            idx = result.find(marker)
            if idx != -1:
                result = result[:idx].strip()
        # Final cleanup: ensure it ends cleanly
        result = result.split(";")[0].strip()
        if result and not result.endswith(";"):
            result += ";"
        # Only return if it starts with SELECT / WITH
        if result and (result.upper().startswith("SELECT") or result.upper().startswith("WITH")):
            return result
    ...
    async def generate_sql(
        ...
        sql = await chat.generate_answer(query, prompt)
    ...
    async def query_with_retry(self, nl_query: str, tenant_id: int) -> SQLResult:
        ...
        sql = self._clean_sql(sql)  # sanitize before first execution
